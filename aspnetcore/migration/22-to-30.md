...
* MVC `Newtonsoft.Json` supporta &ndash; supporto per l'uso di MVC con `Newtonsoft.Json` fa ora parte di [Microsoft. AspNetCore. Mvc. NewtonsoftJson](https://www.nuget.org/packages/Microsoft.AspNetCore.Mvc.NewtonsoftJson).
...
### <a name="systemruntimeserialization-attributes-not-supported"></a>Attributi System. Runtime. Serialization non supportati

Gli attributi [[DataContract]](xref:System.Runtime.Serialization.DataContractAttribute) e [[DataMember]](xref:System.Runtime.Serialization.DataMemberAttribute) non sono supportati in `System.Text.Json`. Per altre informazioni, vedere il [supporto per il problema System. Text. JSON di GitHub in System. Runtime. Serialization](https://github.com/dotnet/corefx/issues/38758).
...
Se si usano le funzionalità di `Newtonsoft.Json` che non sono supportate in `System.Text.Json`, è possibile tornare al `Newtonsoft.Json`:
...
1. Nel client, concatenare una chiamata al metodo `AddNewtonsoftJsonProtocol` all'istanza di `HubConnectionBuilder`:
