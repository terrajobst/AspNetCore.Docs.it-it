...
È possibile che l'app richieda l'integrazione `Newtonsoft.Json` se usa funzionalità specifiche di `Newtonsoft.Json`, ad esempio JsonPatch o convertitori, oppure se [Formatta](xref:web-api/advanced/formatting) i tipi specifici di `Newtonsoft.Json`.
...
In ASP.NET Core 3,0 ASP.NET Core MVC rimuove il suffisso `Async` dai nomi delle azioni del controller. Questo nuovo valore predefinito è influenzato dalla generazione del routing e del collegamento. Esempio:
...
Come illustrato nella documentazione relativa alle [differenze rispetto alle versioni precedenti del routing](xref:fundamentals/routing#differences-from-earlier-versions-of-routing), esistono alcune differenze nella generazione dei collegamenti, ad esempio usando `Url.Link` e API simili. tra cui:
...
1. Installare [Microsoft. AspNetCore.SignalR. Pacchetto NuGet Protocols. NewtonsoftJson](https://www.nuget.org/packages/Microsoft.AspNetCore.SignalR.Protocols.NewtonsoftJson) .
...
Per le app che usano <xref:Microsoft.AspNetCore.TestHost.TestServer> direttamente con l' [host generico](xref:fundamentals/host/web-host), creare il `TestServer` in un <xref:Microsoft.AspNetCore.Hosting.IWebHostBuilder> in <xref:Microsoft.Extensions.Hosting.GenericHostWebHostBuilderExtensions.ConfigureWebHost*>:
...
* [Alcune impostazioni predefinite di navigava sullostesso sito cookie sono state modificate in None (ASPNET/annunciations #348)](https://github.com/aspnet/Announcements/issues/348).
