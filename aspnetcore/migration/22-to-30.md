...
* Impostare il [Target Framework Moniker (TFM)](/dotnet/standard/frameworks) a `netcoreapp3.0`:
...
* Aggiornamento di `Version` attributo rimanenti `<PackageReference>` elementi per `Microsoft.AspNetCore.*` pacchetti per l'anteprima corrente (ad esempio, `3.0.0-preview5-19227-01`).

  Se è presente alcuna versione 3.0 di un pacchetto, il pacchetto potrebbe essere stato deprecato nella versione 3.0. Molti di questi pacchetti sono parte di `Microsoft.AspNetCore.App` e non deve essere specificato singolarmente. Per un elenco preliminare dei pacchetti non più prodotti in 3.0, vedere [arrestare la produzione di pacchetti per gli assembly di framework condiviso nella versione 3.0 (aspnet/AspNetCore #3756)](https://github.com/aspnet/AspNetCore/issues/3756). Il *framework condiviso* è il set di assembly (*DLL* file) che vengono installati nel computer e fa riferimento `Microsoft.AspNetCore.App`. Per altre informazioni, vedere [The shared framework](https://natemcmaster.com/blog/2018/08/29/netcore-primitives-2/) (Il framework condiviso).

* Gli assembly per vari componenti rilevanti sono stati rimossi da `Microsoft.AspNetCore.App` nella versione 3.0. Aggiungere `<PackageReference>` elementi, se si usano le API da pacchetti elencati nel [gli assembly da rimuovere da 3.0 Microsoft.AspNetCore.App (aspnet/AspNetCore #3755)](https://github.com/aspnet/AspNetCore/issues/3755).

  Esempi di componenti rimossi:

  * `Microsoft.AspNet.WebApi.Client`
  * `Microsoft.EntityFrameworkCore`
  * `System.Data.SqlClient`

  L'elenco degli assembly di spedizione `Microsoft.AspNetCore.App` non è ancora stata finalizzata e cambierà prima 3.0 RTM.
  ...
  Il `ReadAsAsync` metodo chiamato nel codice precedente è incluso in `Microsoft.AspNet.WebApi.Client`. Installare il [ASPNET](https://www.nuget.org/packages/Microsoft.AspNet.WebApi.Client/) pacchetto NuGet per risolvere il problema di compilazione nella versione 3.0.
...
* Update `Startup.ConfigureServices` chiamare `AddNewtonsoftJson`.
  ...
  * `AddControllers`
  ...
  Json.NET impostazioni possono essere impostate nella chiamata a `AddNewtonsoftJson`:
...
Nelle versioni precedenti di ASP.NET Core, supporto per l'autorizzazione è stato fornito tramite il `[Authorize]` attributo. Middleware di autorizzazione non era disponibile. In ASP.NET Core 3.0, il middleware di autorizzazione è necessario. È consigliabile posizionare il Middleware di autorizzazione di ASP.NET Core (`UseAuthorization`) subito dopo `UseAuthentication`. Il Middleware di autorizzazione può essere configurato anche con un criterio predefinito, che può essere sottoposto a override.
...
`System.Text.Json` è ora il valore predefinito dell'Hub protocollo usato dal client e server.

Nelle `Startup.ConfigureServices`, chiamare `AddJsonProtocol` per impostare opzioni del serializzatore.
...
Se si usano funzionalità del `Newtonsoft.Json` che non sono supportati in `System.Text.Json`, è possibile tornare alla `Newtonsoft.Json`:

1. Installare il [Microsoft.AspNetCore.SignalR.Protocols.NewtonsoftJson](https://www.nuget.org/packages/Microsoft.AspNetCore.SignalR.Protocols.NewtonsoftJson) pacchetto NuGet.
1. Sul client, concatenare un' `AddNewtonsoftJsonProtocol` chiamata al metodo il `HubConnectionBuilder` istanza:
...
1. Nel server, concatenare un' `AddNewtonsoftJsonProtocol` chiamata al metodo per il `AddSignalR` chiamata al metodo `Startup.ConfigureServices`:
...
In 3.0, la compilazione di runtime è uno scenario di consenso esplicito. Per abilitare la compilazione di runtime, vedere https://docs.microsoft.com/aspnet/core/mvc/views/view-compilation?view=aspnetcore-3.0#runtime-compilation.
