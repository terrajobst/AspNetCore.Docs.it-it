...
I progetti destinati a .NET Core devono usare la [TFM](/dotnet/standard/frameworks) di una versione maggiore o uguale a .NET Core versione 2.2. Nel file di progetto, aggiornare il `<TargetFramework>` testo interno del nodo con `netcoreapp2.2`:
...
Adottare il [modello di hosting in-process per IIS](xref:host-and-deploy/iis/index#in-process-hosting-model), aggiungere il `<AspNetCoreHostingModel>` proprietà con il valore `InProcess` a un `<PropertyGroup>` nel file di progetto:
...
## <a name="update-a-custom-webconfig-file"></a>Aggiornare un file Web. config personalizzata

Per i progetti che usano una classe personalizzata *Web. config* file nella radice del progetto per generare i pubblicata *Web. config* file:

* Nel `<handlers>` voce che aggiunge il modulo ASP.NET Core (`name="aspNetCore"`), modificare il `modules` dal valore dell'attributo `AspNetCoreModule` a `AspNetCoreModuleV2`.
* Nel `<aspNetCore>` elemento, aggiungere l'attributo del modello di hosting (`hostingModel="InProcess"`).

Per ulteriori informazioni ed esempi *Web. config* i file, vedere <xref:host-and-deploy/aspnet-core-module#configuration-with-webconfig>.
