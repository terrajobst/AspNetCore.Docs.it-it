...
Se l'app chiama <xref:Microsoft.AspNetCore.Hosting.WebHostBuilderKestrelExtensions.UseKestrel*> nella `CreateDefaultBuilder` metodo del `Program` classe, chiamare `ConfigureKestrel` per configurare il server Kestrel anziché `UseKestrel` per evitare conflitti con il [IIS in-process del modello di hosting](xref:fundamentals/servers/index#in-process-hosting-model):
...
Se l'app non chiama `CreateDefaultBuilder`, chiamare <xref:Microsoft.AspNetCore.Hosting.WebHostBuilderKestrelExtensions.UseKestrel*> **prima** chiamata `ConfigureKestrel`:
...
* Sposta l'inizializzazione del provider di registrazione `Startup.ConfigureServices`.
...
* Modifica il filtro per l'uso di <xref:Microsoft.Extensions.DependencyInjection.LoggingServiceCollectionExtensions.AddLogging*> generatore.
...
* Modificare il caricamento della configurazione per usare il `AddLogging` generatore.
