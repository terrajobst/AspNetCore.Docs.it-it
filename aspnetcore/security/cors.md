...
* Imposta il nome del criterio "\_myAllowSpecificOrigins". Il nome del criterio è arbitrario.
* Le chiamate di <xref:Microsoft.AspNetCore.Builder.CorsMiddlewareExtensions.UseCors*> metodo di estensione, consentendo la condivisione CORS.
...
Il codice evidenziato seguente applica i criteri CORS per tutti gli endpoint di App tramite il Middleware CORS:
...
Il [specifica CORS](https://www.w3.org/TR/cors/) introdotte diverse nuove intestazioni HTTP che attivare richieste multiorigine. Se un browser supporta la condivisione CORS, imposta le intestazioni delle automaticamente per le richieste multiorigine. Il codice JavaScript personalizzato non è necessario abilitare CORS.
...
1. Abilitare la condivisione CORS tramite uno degli approcci in questo documento. Ad esempio:
  ...
  > [!WARNING]
  > `WithOrigins("https://localhost:<port>");` deve essere utilizzato solo per i test di un'app di esempio simile al [scaricare codice di esempio](https://github.com/aspnet/AspNetCore.Docs/tree/live/aspnetcore/security/cors/sample/Cors).
