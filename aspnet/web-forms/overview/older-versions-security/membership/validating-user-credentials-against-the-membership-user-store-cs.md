...
**Figura 1**: interfaccia include due caselle di testo della pagina di accesso di, un controllo CheckBoxList e un pulsante ([fare clic per visualizzare l'immagine con dimensioni normali](validating-user-credentials-against-the-membership-user-store-cs/_static/image3.png))
...
Il [controllo di accesso Web](https://msdn.microsoft.com/library/system.web.ui.webcontrols.login.aspx) esegue il rendering di molto simile a quello creati in un'interfaccia utente predefinita di <a id="SKM5"> </a> [ *una panoramica dell'autenticazione basata su form* ](../introduction/an-overview-of-forms-authentication-cs.md) esercitazione. Utilizzo del controllo di accesso ci consente di risparmiare il lavoro di dover creare l'interfaccia per raccogliere le credenziali s visitatore. Inoltre, il controllo di accesso accede automaticamente all'utente (presupponendo che le credenziali inviate non sono valide), in tal modo il salvataggio di Stati Uniti di dover scrivere alcun codice.
...
**Figura 5**: valori dettare un aspetto delle proprietà del controllo di accesso di relativo ([fare clic per visualizzare l'immagine con dimensioni normali](validating-user-credentials-against-the-membership-user-store-cs/_static/image15.png))
...
**Figura 6**: converte il controllo di accesso a un modello ([fare clic per visualizzare l'immagine con dimensioni normali](validating-user-credentials-against-the-membership-user-store-cs/_static/image18.png))
...
**Figura 8**: ora controllo di accesso include una casella di testo per l'indirizzo di posta elettronica dell'utente ([fare clic per visualizzare l'immagine con dimensioni normali](validating-user-credentials-against-the-membership-user-store-cs/_static/image24.png))
...
**Figura 9**: flusso di lavoro autenticazione sul controllo account di accesso ([fare clic per visualizzare l'immagine con dimensioni normali](validating-user-credentials-against-the-membership-user-store-cs/_static/image27.png))
...
## <a name="step-4-improving-the-login-controls-invalid-credentials-message"></a>Passaggio 4: Migliorare il messaggio di credenziali non valide del controllo di accesso
...
**Figura 11**: Tito eseguita troppo numerosi tentativi non validi account di accesso e ha stati bloccati ([fare clic per visualizzare l'immagine con dimensioni normali](validating-user-credentials-against-the-membership-user-store-cs/_static/image33.png))
