...
- [Guida all'API di SignalR Hubs - Client JavaScript](../guide-to-the-api/hubs-api-guide-javascript-client.md)
...
L'oggetto proxy è thread-safe. In effetti, se si chiama `HubConnection.CreateHubProxy` più volte con lo stesso `hubName`, si ottiene lo stesso memorizzato nella cache `IHubProxy` oggetto.
...
Per altre informazioni, vedere [comprensione e la gestione degli eventi di durata connessione in SignalR](../guide-to-the-api/handling-connection-lifetime-events.md).
