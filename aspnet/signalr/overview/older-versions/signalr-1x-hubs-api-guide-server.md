...
Per definire la route che i client utilizzeranno per connettersi all'Hub di, chiamare il [MapHubs](https://msdn.microsoft.com/library/system.web.routing.signalrrouteextensions.maphubs(v=vs.111).aspx) metodo all'avvio dell'applicazione. `MapHubs`è un [metodo di estensione](https://msdn.microsoft.com/library/vstudio/bb383977.aspx) per la `System.Web.Routing.RouteCollection` classe. Nell'esempio seguente viene illustrato come definire le route degli hub SignalR nella *Global. asax* file.
