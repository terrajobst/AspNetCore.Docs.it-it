...
---
<a name="improving-performance-with-output-caching-c"></a>Miglioramento delle prestazioni con Output la memorizzazione nella cache (c#)
====================
...
Abilitare la memorizzazione nella cache di output aggiungendo un attributo [OutputCache] per un'azione del controller singoli o una classe controller intero. Ad esempio, il controller nel listato 1 espone un'azione denominata index (). L'output dell'azione Index () viene memorizzato nella cache per 10 secondi.
...
Per impostazione predefinita, quando si utilizza l'attributo [OutputCache], il contenuto memorizzato nella cache di tre posizioni: il server web, tutti i server proxy e il web browser. È possibile controllare esattamente in cui contenuto viene memorizzato nella cache modificando la proprietà Location dell'attributo [OutputCache].
...
**Elenco di 3: Controllers\BadUserController.cs**
...
È possibile risolvere questo problema sfruttando la proprietà VaryByParam dell'attributo [OutputCache]. Questa proprietà consente di creare versioni diverse di memorizzati nella cache del contenuto quando un parametro form molto stesso o se il parametro di stringa di query varia.
...
**Nel listato 5 – Controllers\MoviesController.cs**
...
**Elenco 7: Controllers\ProfileController.cs**
