...
- [<span data-ttu-id="4d70b-118">Per ulteriori informazioni</span><span class="sxs-lookup"><span data-stu-id="4d70b-118">For More Information</span></span>](#More_Info)
...
> 
> - <span data-ttu-id="4d70b-296">In Windows Vista o Windows 7, abilitare il controllo dell'account utente.</span><span class="sxs-lookup"><span data-stu-id="4d70b-296">On Windows Vista or Windows 7, enable UAC.</span></span>
...
> 
> <span data-ttu-id="4d70b-322">**Soluzione alternativa** </span><span class="sxs-lookup"><span data-stu-id="4d70b-322">**Workaround** </span></span>  
...
> <span data-ttu-id="4d70b-356">In alcune circostanze, la pubblicazione di un'applicazione genera un errore di "servizio non è disponibile".</span><span class="sxs-lookup"><span data-stu-id="4d70b-356">Under some circumstances, publishing an application causes a "service is unavailable" error.</span></span>
> 
...
> <span data-ttu-id="4d70b-406">Per impostazione predefinita, WebMatrix installa MySQL con il set di caratteri UTF-8.</span><span class="sxs-lookup"><span data-stu-id="4d70b-406">By default, WebMatrix installs MySQL with the UTF-8 character set.</span></span> <span data-ttu-id="4d70b-407">Se è possibile installare MySQL per conto proprio e il set di caratteri non UTF-8 (ad esempio, è Latin1), il processo di pubblicazione per i database potrebbe non riuscire.</span><span class="sxs-lookup"><span data-stu-id="4d70b-407">If you install MySQL on your own, and the character set is not UTF-8 (for example, it is Latin1), the publish process for databases might fail.</span></span>
> 
> <span data-ttu-id="4d70b-408">**Soluzione alternativa**</span><span class="sxs-lookup"><span data-stu-id="4d70b-408">**Workaround**</span></span>
> 
> 1. <span data-ttu-id="4d70b-409">Modificare il set di caratteri per MySQL UTF-8.</span><span class="sxs-lookup"><span data-stu-id="4d70b-409">Change the character set for MySQL to UTF-8.</span></span> <span data-ttu-id="4d70b-410">(Per informazioni dettagliate, vedere [Server Set di caratteri e le regole di confronto](http://dev.mysql.com/doc/refman/5.0/en/charset-server.html) sul sito Web di MySQL.)</span><span class="sxs-lookup"><span data-stu-id="4d70b-410">(For details, see [Server Character Set and Collation](http://dev.mysql.com/doc/refman/5.0/en/charset-server.html) on the MySQL website.)</span></span>
> 2. <span data-ttu-id="4d70b-411">Reinstallare l'applicazione.</span><span class="sxs-lookup"><span data-stu-id="4d70b-411">Reinstall the application.</span></span>
> 3. <span data-ttu-id="4d70b-412">Ripubblicare l'applicazione.</span><span class="sxs-lookup"><span data-stu-id="4d70b-412">Republish the application.</span></span>


#### <a name="issue-download-published-site-fails-for-applications-that-have-browser-based-setup"></a><span data-ttu-id="4d70b-413">Problema: "Sito di Download pubblicato" non riesce per le applicazioni che dispongono di un'installazione basata su browser</span><span class="sxs-lookup"><span data-stu-id="4d70b-413">Issue: "Download published site" fails for applications that have browser-based setup</span></span>

> <span data-ttu-id="4d70b-414">Alcune applicazioni (ad esempio, Kentico CMS) è necessario avviarli nel browser per eseguire il programma di installazione di post-installazione, ad esempio la creazione di un database.</span><span class="sxs-lookup"><span data-stu-id="4d70b-414">Some applications (for example, Kentico CMS) require you to launch them in the browser in order to perform post-installation setup such as creating a database.</span></span> <span data-ttu-id="4d70b-415">Se si pubblica un'applicazione simile al seguente senza completare l'installazione basata su browser, verrà eseguito il tentativo di scaricare lo stesso sito da un server remoto.</span><span class="sxs-lookup"><span data-stu-id="4d70b-415">If you publish an application like this without completing the browser-based setup, attempting to download the same site from a remote server will fail.</span></span>
> 
> <span data-ttu-id="4d70b-416">**Soluzione alternativa**</span><span class="sxs-lookup"><span data-stu-id="4d70b-416">**Workaround**</span></span>  
> <span data-ttu-id="4d70b-417">Completare l'installazione basata su browser prima di pubblicare il sito.</span><span class="sxs-lookup"><span data-stu-id="4d70b-417">Finish browser-based setup before publishing the site.</span></span>


#### <a name="issue-download-published-site-fails-with-a-database-error-for-dotnetnuke-and-kooboo-cms"></a><span data-ttu-id="4d70b-418">Problema: "Sito di Download pubblicato" viene generato un errore di database per DotNetNuke e Kooboo CMS</span><span class="sxs-lookup"><span data-stu-id="4d70b-418">Issue: "Download published site" fails with a database error for DotNetNuke and Kooboo CMS</span></span>

> <span data-ttu-id="4d70b-419">Se si tenta di scaricare un'applicazione da un server e si dispone delle credenziali nella stringa di connessione del database nel **impostazioni di pubblicazione** finestra di dialogo, si potrebbe vedere il seguente errore nel Registro di pubblicazione:</span><span class="sxs-lookup"><span data-stu-id="4d70b-419">If you try to download an application from a server and you have administrator credentials in the database connection string in the **Publish Settings** dialog, you might see the following error in the publish log:</span></span>
> 
> [!code-console[Main](overview/samples/sample9.cmd)]
> 
> <span data-ttu-id="4d70b-420">**Soluzione alternativa**</span><span class="sxs-lookup"><span data-stu-id="4d70b-420">**Workaround**</span></span>  
> <span data-ttu-id="4d70b-421">Se possibile, eseguire di nuovo il sito (o pubblicarlo) usando credenziali senza privilegi di amministratore per il database.</span><span class="sxs-lookup"><span data-stu-id="4d70b-421">If practical, republish the site (or have it published) using non-administrator credentials for the database.</span></span>


<a id="More_Info"></a>

## <a name="for-more-information"></a><span data-ttu-id="4d70b-422">Ulteriori informazioni</span><span class="sxs-lookup"><span data-stu-id="4d70b-422">For More Information</span></span>

<span data-ttu-id="4d70b-423">Per ulteriori informazioni su WebMatrix 1.0, vedere i siti Web seguenti:</span><span class="sxs-lookup"><span data-stu-id="4d70b-423">For more information about WebMatrix 1.0, see the following websites:</span></span>

- [<span data-ttu-id="4d70b-424">IIS.net</span><span class="sxs-lookup"><span data-stu-id="4d70b-424">IIS.net</span></span>](http://iis.net/)
- [<span data-ttu-id="4d70b-425">ASP.NET 2.0</span><span class="sxs-lookup"><span data-stu-id="4d70b-425">ASP.NET</span></span>](https://asp.net/webmatrix)
- [<span data-ttu-id="4d70b-426">Microsoft.com/web</span><span class="sxs-lookup"><span data-stu-id="4d70b-426">Microsoft.com/web</span></span>](https://www.microsoft.com/web)

<span data-ttu-id="4d70b-427">© 2011 Microsoft Corporation.</span><span class="sxs-lookup"><span data-stu-id="4d70b-427">© 2011 Microsoft Corporation.</span></span> <span data-ttu-id="4d70b-428">Tutti i diritti riservati.</span><span class="sxs-lookup"><span data-stu-id="4d70b-428">All Rights Reserved.</span></span> <span data-ttu-id="4d70b-429">[Condizioni di utilizzo](https://msdn.microsoft.cos/cc300389.aspx).</span><span class="sxs-lookup"><span data-stu-id="4d70b-429">[Terms of Use](https://msdn.microsoft.cos/cc300389.aspx).</span></span>
